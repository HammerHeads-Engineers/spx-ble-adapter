...
function normalizeNotifyConfig(config) {
  if (!config) {
    return { triggers: ['state'], intervalMs: 0 };
  }
  const triggers = Array.isArray(config.triggers) && config.triggers.length > 0
    ? config.triggers
    : ['state'];
  const intervalMs = typeof config.intervalMs === 'number' && config.intervalMs > 0
    ? config.intervalMs
    : 0;
  return { triggers, intervalMs };
}
...
function buildCharacteristics(serviceConfig, config, state) {
  ...
  return serviceConfig.characteristics.map((charConfig) => {
    ...
    if (properties.includes('notify')) {
      const notifyConfig = normalizeNotifyConfig(charConfig.notify);
      let updateValueCallback = null;
      let unsubscribeState = null;
      let timerId = null;

      const pushNotification = () => {
        if (!updateValueCallback) return;
        try {
          const resolvedValue = resolveValue(charConfig.value, state);
          const buffer = encodeValue(resolvedValue, charConfig.codec || {});
          updateValueCallback(buffer);
        } catch (err) {
          console.error(`[BLE] notify error (${charConfig.uuid}):`, err.message);
        }
      };

      const subscribeState = () => {
        if (!notifyConfig.triggers.includes('state')) return;
        if (!charConfig.value || charConfig.value.source !== 'state' || !charConfig.value.key) {
          unsubscribeState = state.onAnyChange(() => pushNotification());
        } else {
          unsubscribeState = state.onChange(charConfig.value.key, () => pushNotification());
        }
      };

      options.onSubscribe = (maxValueSize, updateCallback) => {
        updateValueCallback = updateCallback;
        subscribeState();

        if (notifyConfig.triggers.includes('timer') && notifyConfig.intervalMs > 0) {
          timerId = setInterval(() => pushNotification(), notifyConfig.intervalMs);
        }
